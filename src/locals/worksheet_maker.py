...
Pinyin is resolved per character as:
    ...
    2. the curated reading in character_pinyin_constants.py (if a single char)
    3. xpinyin's most common reading

Examples
--------
    # inline characters -> ./worksheet.pdf
    python worksheet_maker.py 花 园 门 前

    # a list file -> ./yr1_character_cards.pdf  (name derived from the file)
    python worksheet_maker.py --file yr1_chars.txt

    # override a 多音字 reading
    python worksheet_maker.py --chars "重(zhòng) 行(háng)"

    # regenerate every *_chars.txt in a folder, writing PDFs beside them
    python worksheet_maker.py --batch worksheets

    # compact grid sheet instead of big cards
    python worksheet_maker.py --file yr1_chars.txt --layout grid --cols 8
...
# Pinyin data lives in the repo's common/ package.
sys.path.insert(0, os.path.join(HERE, "..", "common"))
try:
    from character_pinyin_constants import (  # type: ignore
    ...
    def decode_pinyin(s):  # type: ignore
        return s
...
DEFAULT_CHAR_FONT = os.path.join(HERE, "GB2312.ttf")
