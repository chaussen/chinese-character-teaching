...
from chinese_tools.data.pinyin_data import CHARACTER_PINYIN_ENGLISH_MAPPING
from chinese_tools.cards.flashcard_maker import ChineseCardMaker
from chinese_tools.kahoot.processor import KahootProcessing
from chinese_tools.paths import PACKAGE_DIR
import subprocess
import urllib.parse
...
class Application(tk.Frame):
    ...
    def create_question_sheet(self):
        ...
        kahoot_dir = os.path.join(PACKAGE_DIR, 'kahoot')
        source = os.path.join(kahoot_dir, 'kahoot.txt')
        destination = ''
        if english is True and reverse is False:
            destination = os.path.join(kahoot_dir, '1')
        if english is False and reverse is True:
            destination = os.path.join(kahoot_dir, '2')
        if english is False and reverse is False:
            destination = os.path.join(kahoot_dir, '3')
        if english is True and reverse is True:
            destination = os.path.join(kahoot_dir, '4')
    ...
    def create_xlsx(self):
        ...
        kahoot_dir = os.path.join(PACKAGE_DIR, 'kahoot')
        whole_csv = os.path.join(kahoot_dir, f'{self.question}kahoot.csv')
        self.processing.generate_mixed_questions(
            int(self.question_count.get()),
            whole_csv,
            [
                os.path.join(kahoot_dir, f'{self.question}1.csv'),
                os.path.join(kahoot_dir, f'{self.question}2.csv'),
                os.path.join(kahoot_dir, f'{self.question}3.csv'),
                os.path.join(kahoot_dir, f'{self.question}4.csv')
            ])
